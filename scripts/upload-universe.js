...
const INDUSTRY_RULES = [
  ...
  { group: 'broker', keywords: ['证券'], factor: 0.93, cap: 20 },
  { group: 'realestate', keywords: ['地产', '置业', '万科', '保利', '招商蛇口'], factor: 0.75, cap: 25 },
  { group: 'liquor', keywords: ['茅台', '五粮液', '泸州老窖', '汾酒', '洋河', '古井贡', '舍得', '水井坊', '酒鬼酒'], factor: 1.00, cap: 20 },
  { group: 'food', keywords: ['食品', '乳业', '伊利', '海天', '双汇', '安琪酵母', '桃李面包', '洽洽', '盐津', '安井', '千禾'], factor: 1.00, cap: 20 },
  { group: 'pharmacy', keywords: ['医药', '药业', '生物', '医疗', '健康', '制药', '恒瑞', '药明', '迈瑞', '片仔癀', '云南白药', '同仁堂'], factor: 1.03, cap: 60 },
  { group: 'hightech', keywords: ['半导体', '芯片', '微电子', '集成电路', '中芯', '韦尔', '兆易', '北方华创', '中微'], factor: 1.08, cap: 30 },
  { group: 'tech', keywords: ['科技', '软件', '信息', '通信', '浪潮', '中兴', '用友', '金山', '科大讯飞', '三六零'], factor: 1.05, cap: 40 },
  { group: 'newenergy', keywords: ['新能源', '锂电', '光伏', '风电', '宁德时代', '比亚迪', '隆基', '通威', '阳光电源', '亿纬锂能'], factor: 1.06, cap: 40 },
  { group: 'automotive', keywords: ['汽车', '长城汽车', '赛力斯', '长安汽车', '上汽', '广汽', '吉利', '宇通', '福耀玻璃'], factor: 1.02, cap: 25 },
  { group: 'defense', keywords: ['军工', '航天', '航空', '船舶', '兵器', '中航', '中船', '航发'], factor: 1.04, cap: 25 },
  { group: 'homeappliance', keywords: ['家电', '美的', '格力', '海尔', '苏泊尔', '老板', '九阳', '欧派'], factor: 1.00, cap: 15 },
  { group: 'consumer', keywords: ['消费', '零售', '美妆', '珀莱雅', '中国中免'], factor: 0.98, cap: 20 },
  { group: 'machinery', keywords: ['机械', '装备', '三一', '中联', '徐工', '汇川'], factor: 1.00, cap: 30 },
  { group: 'chemical', keywords: ['化工', '万华', '华鲁', '荣盛', '恒力'], factor: 0.92, cap: 25 },
  { group: 'steel', keywords: ['钢铁', '宝钢', '鞍钢'], factor: 0.88, cap: 15 },
  { group: 'coal', keywords: ['煤炭', '神华', '中煤'], factor: 0.88, cap: 10 },
  { group: 'power', keywords: ['电力', '长江电力', '华能', '国电', '核电'], factor: 0.95, cap: 25 },
  { group: 'construction', keywords: ['建筑', '建设', '中铁', '中建', '中铁建', '交建'], factor: 0.88, cap: 20 },
  { group: 'transport', keywords: ['航空', '机场', '航运', '港口', '高速', '铁路', '物流', '顺丰', '中通'], factor: 0.95, cap: 25 },
  { group: 'agriculture', keywords: ['农业', '种子', '牧原', '温氏', '新希望', '海大'], factor: 0.95, cap: 15 },
  { group: 'textile', keywords: ['纺织', '服装', '申洲'], factor: 0.93, cap: 10 },
  { group: 'media', keywords: ['传媒', '影视', '游戏', '分众', '芒果'], factor: 0.95, cap: 15 },
];
...
function scoreStocks(stocks) {
  ...
  for (const s of filtered) {
    ...
    if ((s.circ_mv && s.circ_mv >= 100) || (s.total_mv && s.total_mv >= 200)) {
      if (s._liqScore >= 30) addStock(s, 'core');
    }
  }
  for (const s of filtered) {
    if (selected.length >= 600) break;
    if (s._liqScore >= 40 && s._riskScore >= 30) addStock(s, 'core-supplement');
  }
  ...
  for (const [group, stocks] of Object.entries(sectorGroups)) {
    ...
    const quota = Math.max(2, Math.min(stocks.length, Math.round(stocks.length / filtered.length * 300 * 1.5)));
  ...
  for (const s of filtered) {
    if (selected.length >= 900) break;
    if (s._adjustedScore >= 40) addStock(s, 'sector-supplement');
  }
  console.log(`Layer 2 (sector): ${selected.length - 600} stocks, total ${selected.length}`);

  // Layer 3: 成长活跃 100只
  const growthGroups = new Set(['growth', 'hightech', 'tech', 'newenergy', 'healthcare', 'defense', 'pharmacy']);
  ...
  for (const s of filtered) {
    if (selected.length >= 1000) break;
    if (s._adjustedScore >= 35 && s._riskScore >= 30) addStock(s, 'supplement');
  }
...
async function uploadToServer(stocks) {
  ...
  for (let i = 0; i < stocks.length; i += BATCH) {
    ...
    try {
      ...
      console.log(`  Uploaded ${uploaded}/${stocks.length}: ${res.data?.ok ? 'OK' : JSON.stringify(res.data)}`);
    } catch (e) {
      console.error(`  Upload batch ${Math.floor(i/BATCH)+1} failed:`, e.message?.substring(0, 200));
      if (e.response) console.error('    Response:', e.response.data?.error || e.response.status);
    }
    await sleep(200);
  }
  return uploaded;
